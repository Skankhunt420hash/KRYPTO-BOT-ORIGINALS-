from typing import TYPE_CHECKING, Dict, List, Optional

from config.settings import settings
from src.strategies.signal import EnhancedSignal, Side
from src.engine.regime import Regime
from src.engine.runtime_control import runtime_control
from src.utils.logger import setup_logger

if TYPE_CHECKING:
    from src.engine.strategy_scorer import StrategyScorer

logger = setup_logger("meta_selector")

# Gewicht des Performance-Scores im Gesamt-Score.
# final = signal_score + (perf_score - 0.5) * PERF_WEIGHT
# Bei PERF_WEIGHT=0.15: max Anpassung = ±0.075 (konservativ)
_PERF_WEIGHT: float = settings.PERF_SELECTOR_WEIGHT

# Richtungs-Multiplikator: penalisiert Signale die gegen das Regime laufen.
# SHORT im TREND_UP = kontra-zyklisch = 0.55 Abzug auf den Fit-Score.
# LONG im TREND_DOWN = kontra-zyklisch = 0.55 Abzug.
# In den meisten nicht-direktionalen Regimes sind beide Richtungen ähnlich gewichtet.
DIRECTION_MULTIPLIER: Dict[Regime, Dict[Side, float]] = {
    Regime.TREND_UP:        {Side.LONG: 1.00, Side.SHORT: 0.55, Side.NONE: 0.0},
    Regime.TREND_DOWN:      {Side.LONG: 0.55, Side.SHORT: 1.00, Side.NONE: 0.0},
    Regime.TREND_MARKET:    {Side.LONG: 1.00, Side.SHORT: 1.00, Side.NONE: 0.0},
    Regime.SIDEWAYS_MARKET: {Side.LONG: 1.00, Side.SHORT: 1.00, Side.NONE: 0.0},
    Regime.MANIPULATION_PHASE: {Side.LONG: 0.95, Side.SHORT: 0.95, Side.NONE: 0.0},
    Regime.NEWS_SHOCK:      {Side.LONG: 1.00, Side.SHORT: 1.00, Side.NONE: 0.0},
    Regime.LOW_VOL_TRAP:    {Side.LONG: 1.00, Side.SHORT: 1.00, Side.NONE: 0.0},
    Regime.PUMP_DUMP_RISK:  {Side.LONG: 0.95, Side.SHORT: 0.95, Side.NONE: 0.0},
    Regime.LIQUIDATION_CASCADE: {Side.LONG: 1.00, Side.SHORT: 1.00, Side.NONE: 0.0},
    Regime.RANGE:           {Side.LONG: 1.00, Side.SHORT: 1.00, Side.NONE: 0.0},
    Regime.HIGH_VOLATILITY: {Side.LONG: 1.00, Side.SHORT: 1.00, Side.NONE: 0.0},
    Regime.LOW_VOLATILITY:  {Side.LONG: 1.00, Side.SHORT: 1.00, Side.NONE: 0.0},
}

# Regime-Fit-Tabelle: je höher der Wert, desto besser passt die Strategie zum Regime.
# Neue Strategien haben primäre Werte, Legacy-Strategien werden ebenfalls bewertet.
REGIME_STRATEGY_FIT: Dict[Regime, Dict[str, float]] = {
    Regime.TREND_UP: {
        "TrendContinuation": 1.00,
        "MomentumPullback": 0.90,
        "EMAReclaimBreakout": 0.86,
        "RSI_MACD_Confluence": 0.78,
        "MACD_Crossover": 0.70,
        "KeltnerChannelBreakout": 0.64,
        "VolatilityBreakout": 0.50,
        "LiquiditySweepReversal": 0.35,
        "StochRsiMeanReversion": 0.18,
        "StochRSIMeanReversion": 0.18,
        "RangeReversion": 0.12,
        "RSI_EMA": 0.60,
        "Combined": 0.65,
    },
    Regime.TREND_DOWN: {
        "TrendContinuation": 0.80,
        "MomentumPullback": 0.70,
        "EMAReclaimBreakout": 0.72,
        "RSI_MACD_Confluence": 0.70,
        "MACD_Crossover": 0.50,
        "KeltnerChannelBreakout": 0.58,
        "VolatilityBreakout": 0.50,
        "LiquiditySweepReversal": 0.42,
        "StochRsiMeanReversion": 0.16,
        "StochRSIMeanReversion": 0.16,
        "RangeReversion": 0.12,
        "RSI_EMA": 0.40,
        "Combined": 0.45,
    },
    Regime.TREND_MARKET: {
        "TrendContinuation": 1.00,
        "MomentumPullback": 0.92,
        "EMAReclaimBreakout": 0.90,
        "RSI_MACD_Confluence": 0.74,
        "MACD_Crossover": 0.70,
        "KeltnerChannelBreakout": 0.66,
        "VolatilityBreakout": 0.52,
        "LiquiditySweepReversal": 0.40,
        "RangeReversion": 0.16,
        "StochRsiMeanReversion": 0.20,
        "StochRSIMeanReversion": 0.20,
        "RSI_EMA": 0.60,
        "Combined": 0.66,
    },
    Regime.RANGE: {
        "RangeReversion": 1.00,
        "StochRsiMeanReversion": 0.88,
        "StochRSIMeanReversion": 0.88,
        "LiquiditySweepReversal": 0.80,
        "RSI_MACD_Confluence": 0.68,
        "EMAReclaimBreakout": 0.56,
        "MomentumPullback": 0.40,
        "KeltnerChannelBreakout": 0.38,
        "VolatilityBreakout": 0.32,
        "TrendContinuation": 0.20,
        "RSI_EMA": 0.65,
        "MACD_Crossover": 0.30,
        "Combined": 0.50,
    },
    Regime.SIDEWAYS_MARKET: {
        "RangeReversion": 1.00,
        "StochRsiMeanReversion": 0.92,
        "StochRSIMeanReversion": 0.92,
        "LiquiditySweepReversal": 0.82,
        "RSI_MACD_Confluence": 0.70,
        "EMAReclaimBreakout": 0.58,
        "KeltnerChannelBreakout": 0.34,
        "MomentumPullback": 0.34,
        "VolatilityBreakout": 0.24,
        "TrendContinuation": 0.18,
        "RSI_EMA": 0.68,
        "MACD_Crossover": 0.32,
        "Combined": 0.54,
    },
    Regime.MANIPULATION_PHASE: {
        "LiquiditySweepReversal": 1.00,
        "RangeReversion": 0.84,
        "StochRsiMeanReversion": 0.78,
        "StochRSIMeanReversion": 0.78,
        "EMAReclaimBreakout": 0.58,
        "RSI_MACD_Confluence": 0.54,
        "KeltnerChannelBreakout": 0.46,
        "VolatilityBreakout": 0.40,
        "MomentumPullback": 0.32,
        "TrendContinuation": 0.24,
        "RSI_EMA": 0.52,
        "MACD_Crossover": 0.35,
        "Combined": 0.50,
    },
    Regime.NEWS_SHOCK: {
        "VolatilityBreakout": 1.00,
        "KeltnerChannelBreakout": 0.94,
        "TrendContinuation": 0.74,
        "MomentumPullback": 0.64,
        "EMAReclaimBreakout": 0.58,
        "LiquiditySweepReversal": 0.52,
        "RSI_MACD_Confluence": 0.48,
        "RangeReversion": 0.12,
        "StochRsiMeanReversion": 0.10,
        "StochRSIMeanReversion": 0.10,
        "RSI_EMA": 0.22,
        "MACD_Crossover": 0.42,
        "Combined": 0.38,
    },
    Regime.HIGH_VOLATILITY: {
        "VolatilityBreakout": 1.00,
        "KeltnerChannelBreakout": 0.88,
        "LiquiditySweepReversal": 0.72,
        "MomentumPullback": 0.70,
        "EMAReclaimBreakout": 0.62,
        "RSI_MACD_Confluence": 0.48,
        "TrendContinuation": 0.50,
        "RangeReversion": 0.10,
        "StochRsiMeanReversion": 0.14,
        "StochRSIMeanReversion": 0.14,
        "RSI_EMA": 0.30,
        "MACD_Crossover": 0.40,
        "Combined": 0.35,
    },
    Regime.PUMP_DUMP_RISK: {
        "LiquiditySweepReversal": 1.00,
        "KeltnerChannelBreakout": 0.92,
        "VolatilityBreakout": 0.88,
        "TrendContinuation": 0.56,
        "MomentumPullback": 0.46,
        "EMAReclaimBreakout": 0.38,
        "RSI_MACD_Confluence": 0.30,
        "RangeReversion": 0.14,
        "StochRsiMeanReversion": 0.12,
        "StochRSIMeanReversion": 0.12,
        "RSI_EMA": 0.20,
        "MACD_Crossover": 0.34,
        "Combined": 0.32,
    },
    Regime.LIQUIDATION_CASCADE: {
        "VolatilityBreakout": 1.00,
        "TrendContinuation": 0.90,
        "KeltnerChannelBreakout": 0.86,
        "MomentumPullback": 0.76,
        "LiquiditySweepReversal": 0.62,
        "EMAReclaimBreakout": 0.56,
        "RSI_MACD_Confluence": 0.42,
        "RangeReversion": 0.10,
        "StochRsiMeanReversion": 0.08,
        "StochRSIMeanReversion": 0.08,
        "RSI_EMA": 0.18,
        "MACD_Crossover": 0.36,
        "Combined": 0.34,
    },
    Regime.LOW_VOL_TRAP: {
        "RangeReversion": 0.92,
        "StochRsiMeanReversion": 1.00,
        "StochRSIMeanReversion": 1.00,
        "RSI_MACD_Confluence": 0.74,
        "LiquiditySweepReversal": 0.64,
        "EMAReclaimBreakout": 0.48,
        "KeltnerChannelBreakout": 0.22,
        "MomentumPullback": 0.18,
        "VolatilityBreakout": 0.10,
        "TrendContinuation": 0.16,
        "RSI_EMA": 0.58,
        "MACD_Crossover": 0.54,
        "Combined": 0.56,
    },
    Regime.LOW_VOLATILITY: {
        "RangeReversion": 0.85,
        "StochRsiMeanReversion": 0.90,
        "StochRSIMeanReversion": 0.90,
        "RSI_MACD_Confluence": 0.70,
        "LiquiditySweepReversal": 0.62,
        "EMAReclaimBreakout": 0.50,
        "TrendContinuation": 0.30,
        "MomentumPullback": 0.20,
        "VolatilityBreakout": 0.10,
        "KeltnerChannelBreakout": 0.18,
        "RSI_EMA": 0.55,
        "MACD_Crossover": 0.55,
        "Combined": 0.55,
    },
}

# Mindest-Regime-Fit damit ein Signal überhaupt in die Bewertung kommt
MIN_REGIME_FIT: float = 0.35


class MetaSelector:
    """
    Bewertet alle eingehenden Signale und wählt das beste für den aktuellen
    Markt aus.

    Basis-Score-Formel:
        signal_score = regime_fit * 0.35 + confidence_norm * 0.35
                     + rr_score * 0.20 + volume_bonus * 0.10

    Optionale Performance-Anpassung (wenn StrategyScorer übergeben):
        final_score = signal_score + (perf_score - 0.5) * PERF_WEIGHT
        → max ±0.075 bei PERF_WEIGHT=0.15 (konservativ)
        → nur aktiv wenn genug historische Trades vorhanden
    """

    def __init__(self, scorer: Optional["StrategyScorer"] = None):
        self._scorer = scorer
        self._last_selection: Dict = {}

    def set_scorer(self, scorer: "StrategyScorer") -> None:
        """Setzt oder ersetzt den StrategyScorer (nach Initialisierung)."""
        self._scorer = scorer

    def select(
        self,
        signals: List[EnhancedSignal],
        regime: Regime,
        symbol: str,
    ) -> Optional[EnhancedSignal]:
        self._last_selection = {
            "symbol": symbol,
            "regime": regime.value,
            "candidates_total": len(signals),
            "actionable": 0,
            "eligible": 0,
            "blocked_regime": 0,
            "blocked_perf": 0,
            "winner": None,
            "winner_score": None,
        }
        actionable = [
            s for s in signals
            if s.symbol == symbol and s.is_actionable()
        ]
        self._last_selection["actionable"] = len(actionable)

        if not actionable:
            logger.debug(
                f"{symbol} | Kein aktionsfähiges Signal "
                f"({len(signals)} Strategien liefen, Regime: {regime.value})"
            )
            return None

        regime_fits = REGIME_STRATEGY_FIT.get(regime, REGIME_STRATEGY_FIT.get(Regime.RANGE, {}))
        scored: List[tuple] = []
        ctrl = runtime_control.get_snapshot()
        preferred = (ctrl.get("preferred_strategy") or "").strip()
        pref_bonus = settings.CONTROL_STRATEGY_PRIORITY_BONUS if preferred else 0.0

        dir_mults = DIRECTION_MULTIPLIER.get(regime, DIRECTION_MULTIPLIER.get(Regime.RANGE, {}))

        for sig in actionable:
            base_fit = regime_fits.get(sig.strategy_name, 0.30)
            dir_mult = dir_mults.get(sig.side, 1.0)
            fit = base_fit * dir_mult

            if fit < MIN_REGIME_FIT:
                self._last_selection["blocked_regime"] += 1
                logger.debug(
                    f"[REGIME-FILTER] {sig.strategy_name} [{sig.side.value.upper()}] "
                    f"| {symbol} | fit={fit:.2f} (base={base_fit:.2f} × dir={dir_mult:.2f}) "
                    f"< {MIN_REGIME_FIT} – blockiert"
                )
                continue

            conf_score = sig.confidence / 100.0
            rr_score = min(sig.rr / 5.0, 1.0)
            vol_bonus = 0.10 if sig.volume_confirmed else 0.0

            signal_score = (
                fit * 0.35
                + conf_score * 0.35
                + rr_score * 0.20
                + vol_bonus * 0.10
            )

            # Performance-Anpassung (optional, konservativ) und Eligibility-Gate
            perf_score = 0.5   # neutral default
            perf_adj = 0.0
            if self._scorer is not None:
                try:
                    perf_score = self._scorer.get_score(
                        sig.strategy_name, regime.value
                    )
                    # Optionaler harter Gate: Strategien mit dauerhaft sehr
                    # schwacher Performance können komplett ignoriert werden.
                    # Standard: deaktiviert (STRATEGY_MIN_PERF_SCORE = 0.0).
                    if settings.STRATEGY_MIN_PERF_SCORE > 0.0 and perf_score < settings.STRATEGY_MIN_PERF_SCORE:
                        self._last_selection["blocked_perf"] += 1
                        logger.debug(
                            f"[PERF-GATE] {sig.strategy_name} | Regime={regime.value} | "
                            f"perf_score={perf_score:.2f} < "
                            f"min={settings.STRATEGY_MIN_PERF_SCORE:.2f} – Signal ignoriert"
                        )
                        continue

                    if _PERF_WEIGHT > 0:
                        perf_adj = (perf_score - 0.5) * _PERF_WEIGHT
                except Exception as e:
                    logger.warning(
                        f"Scorer.get_score fehlgeschlagen für {sig.strategy_name}: "
                        f"{type(e).__name__} – neutraler Score verwendet"
                    )

            priority_adj = 0.0
            if preferred and sig.strategy_name.lower() == preferred.lower():
                priority_adj = pref_bonus

            total = signal_score + perf_adj + priority_adj
            # Speichere signal_score + perf_adj für Logging des Gewinners
            scored.append((total, sig, signal_score, perf_adj, perf_score, priority_adj))
            logger.debug(
                f"  {sig.strategy_name:<22} [{sig.side.value.upper():<5}] | "
                f"fit={fit:.2f} conf={sig.confidence:.0f} rr={sig.rr:.2f} "
                f"vol={'✓' if sig.volume_confirmed else '✗'} "
                f"sig={signal_score:.3f} perf={perf_score:.2f} adj={perf_adj:+.3f} "
                f"pref={priority_adj:+.3f} "
                f"→ final={total:.3f}"
            )

        if not scored:
            logger.debug(
                f"{symbol} | Alle Signale durch Regime-Filter blockiert "
                f"(Regime: {regime.value})"
            )
            return None
        self._last_selection["eligible"] = len(scored)

        scored.sort(key=lambda x: x[0], reverse=True)
        best_score, best, best_sig_score, best_perf_adj, best_perf_score, best_pref_adj = scored[0]

        # Logging: signal_score + perf_adj + final für den Gewinner
        if self._scorer is not None and _PERF_WEIGHT > 0:
            perf_tag = (
                f" | sig={best_sig_score:.3f} "
                f"perf={best_perf_score:.2f} adj={best_perf_adj:+.3f} "
                f"pref={best_pref_adj:+.3f}"
            )
        else:
            perf_tag = ""

        logger.info(
            f"[cyan]META-SELECTOR[/cyan] {symbol} | "
            f"Gewinner: [bold]{best.strategy_name}[/bold] | "
            f"final={best_score:.3f}{perf_tag} | "
            f"Regime={regime.value} | Seite={best.side.value.upper()} | "
            f"conf={best.confidence:.0f} | RR={best.rr:.2f} | "
            f"{best.reason}"
        )
        self._last_selection["winner"] = best.strategy_name
        self._last_selection["winner_score"] = round(best_score, 4)
        self._last_selection["winner_side"] = best.side.value
        self._last_selection["winner_confidence"] = round(best.confidence, 1)
        self._last_selection["winner_rr"] = round(best.rr, 2)
        return best

    def get_last_selection(self) -> Dict:
        return dict(self._last_selection)
