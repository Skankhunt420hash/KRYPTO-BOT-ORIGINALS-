...
def _tail_log(path: Path, max_lines: int, max_bytes: int = 1_048_576) -> List[str]:
    if not path.is_file():
        return []
    ...
    try:
        ...
        with path.open("rb") as fh:
            ...
            while pos > 0 and bytes_read < max_bytes and newline_count <= max_lines:
                read_size = min(64 * 1024, pos, max_bytes - bytes_read)
                pos -= read_size
                fh.seek(pos)
...
def _resolve_log_path(root: Path) -> Optional[Path]:
    explicit = os.environ.get("SAFETY_WATCHDOG_LOG_FILE")
    if explicit is not None:
        log_rel = explicit.strip()
        if not log_rel:
            return None
    else:
...
def run_forever() -> None:
    ...
    logger.info(
        ...
        poll,
        log_path if log_path else "disabled",
