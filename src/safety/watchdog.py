...
def _tail_log(path: Path, max_lines: int) -> List[str]:
    ...
    try:
        chunk_size = 8192
        max_bytes = max(chunk_size, max_lines * 512)
        chunks: List[bytes] = []
        bytes_read = 0
...
def _maintenance_active(root: Path) -> bool:
    return (root / ".deploy-sync-in-progress").exists()


def _resolve_log_path(root: Path) -> Optional[Path]:
    raw_log_setting = getattr(settings, "SAFETY_WATCHDOG_LOG_FILE", "")
    if "SAFETY_WATCHDOG_LOG_FILE" in os.environ and str(raw_log_setting).strip() == "":
        return None
    log_rel = str(raw_log_setting or "").strip() or getattr(
...
def run_forever() -> None:
    ...
    while True:
        try:
            ...
            if _maintenance_active(root):
                logger.info("Maintenance aktiv — Prozess-/Log-Neustarts uebersprungen")
                time.sleep(poll)
                continue
