...
def _tail_log(path: Path, max_lines: int) -> List[str]:
    if max_lines <= 0 or not path.is_file():
        return []
    try:
        # Von EOF in begrenzten Blöcken lesen; Produktionslogs können sehr groß sein.
...
def _read_new_log_lines(
    ...
    try:
        ...
        rotated = (
            ...
            and (
                current_identity[:2] != identity[:2]
                or not current_identity[2].startswith(identity[2])
            )
        )
...
def _resolve_log_path(root: Path) -> Optional[Path]:
    ...
    log_rel = (
        getattr(settings, "SUPERVISOR_BOT_LOGFILE", "logs/bot_process.log")
        if log_env is None
        else log_env.strip()
    )
