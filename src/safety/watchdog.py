...
def _tail_log(path: Path, max_lines: int) -> List[str]:
    ...
    if max_lines <= 0:
        return []
    try:
        # Bot logs can be large; read backwards in bounded chunks instead of
        # loading the whole file into the watchdog process.
        max_bytes = max(1024 * 1024, max_lines * 4096)
        chunks: List[bytes] = []
        bytes_read = 0
        newline_count = 0
...
def _resolve_log_path(root: Path) -> Optional[Path]:
    if "SAFETY_WATCHDOG_LOG_FILE" in os.environ:
        explicit = os.environ.get("SAFETY_WATCHDOG_LOG_FILE", "").strip()
        if not explicit:
            return None
        log_rel = explicit
    else:
