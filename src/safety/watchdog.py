...
_ERROR_RE = re.compile(
    r"((?<![A-Z0-9_])(ERROR|CRITICAL)(?![A-Z0-9_])|Traceback|Exception:|Fatal Python error)",
    re.IGNORECASE,
)
...
def _tail_log(
    ...
    *,
    ...
    if max_bytes is None:
        max_bytes = max(64 * 1024, max_lines * chunk_size)
    max_bytes = max(chunk_size, int(max_bytes))
    try:
