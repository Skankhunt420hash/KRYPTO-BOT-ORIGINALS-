...
def _resolve_log_path(root: Path) -> Optional[Path]:
    raw = str(getattr(settings, "SAFETY_WATCHDOG_LOG_FILE", "") or "").strip()
    configured = bool(getattr(settings, "SAFETY_WATCHDOG_LOG_FILE_CONFIGURED", False))
    if configured and not raw:
        return None
    log_rel = raw or getattr(settings, "SUPERVISOR_BOT_LOGFILE", "logs/bot_process.log")
...
def run_forever() -> None:
    ...
    logger.info(
        ...
        poll,
        str(log_path) if log_path is not None else "disabled",
