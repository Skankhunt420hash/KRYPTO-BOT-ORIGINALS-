...
_ERROR_RE = re.compile(
    ...
    r"Traceback \(most recent call last\):|Exception:|Fatal Python error",
    re.IGNORECASE,
)
...
def _tail_log(path: Path, max_lines: int) -> List[str]:
    if not path.is_file():
        return []
    try:
        max_bytes = max(4096, int(max_lines) * 512)
...
def run_forever() -> None:
    ...
    if log_env is not None and not log_env.strip():
        log_path = None
    else:
        log_rel = (log_env or getattr(settings, "SAFETY_WATCHDOG_LOG_FILE", "") or "").strip() or getattr(
