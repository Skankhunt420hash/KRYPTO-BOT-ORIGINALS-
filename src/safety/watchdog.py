...
_ERROR_RE = re.compile(
    r"(\b(CRITICAL|Traceback|Exception:|Fatal Python error)\b|(^|\s|\[)ERROR(\]|\s|:))",
    re.IGNORECASE,
)
...
def run_forever() -> None:
    ...
    if raw_log_env is not None and raw_log_env.strip() == "":
        log_path: Optional[Path] = None
    else:
        log_rel = (
            raw_log_env.strip()
            if raw_log_env is not None
            else getattr(settings, "SUPERVISOR_BOT_LOGFILE", "logs/bot_process.log")
        )
